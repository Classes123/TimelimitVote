	...
	switch(eAction)
	{
		...
		case MenuAction_Select:
		{
			if(!g_bHideVotes)
			{
				...
				CPrint(TO_ALL, "player_x_chose_y_minutes", " %t %t", "prefix", "player_x_chose_y_minutes", szName, szTime);
			}
		}
	}
...
void hMenu_VoteCallback(Menu hMenu, int iNumVotes, int iNumClients, const int[][] iClientInfo, int iNumItems, const int[][] iItemInfo)
{
	...
	CPrint(TO_ALL, "timelimit_has_been_changed_to_x_minutes", " %t %t", "prefix", "timelimit_has_been_changed_to_x_minutes", fTime);
}
